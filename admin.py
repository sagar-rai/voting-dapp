...
def deployContract(noProp):
	...
	construct_txn=ballot.constructor(noProp).buildTransaction({
...
def register(addr):
	...
	account=web3.eth.account.privateKeyToAccount(key)
	account_addr=account.address
	web3.eth.defaultAccount=Web3.toChecksumAddress(account_addr)
	ballot=web3.eth.contract(address=addr,abi=abi)
	num=int(input("Enter num"))
	for i in range(num):
		reg_acc=input("Enter reg_acc")
		tx_hash=ballot.functions.register(reg_acc).transact()
		web3.eth.waitForTransactionReceipt(tx_hash)

def handleEvent1(addr):
    receipt = web3.eth.waitForTransactionReceipt(event['transactionHash'])
    ballot=web3.eth.contract(address=addr,abi=abi)
    result = contract.events.voting.processReceipt(receipt)
    print(result[0]['args'])
